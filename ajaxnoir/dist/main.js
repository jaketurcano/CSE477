...
const MovieInfo = function(sel) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default.a.ajax({
        ...
        data: {api_key: "74ceaffbb60e5a217cb7f92249f6ef5e", query: title, year: year},
        method: "GET",
        dataType: "text",
        success: function(data) {
            ...
            that.displayMovieInfo(json, obj);
        },
        error: function(xhr, status, error) {
            // Error
            console.log("Error: "+error);
            console.log('<p>Unable to communicate<br>with themoviedb.org</p>');
        }
    });
}

MovieInfo.prototype.displayMovieInfo = function(json, obj) {
    var html;
    if (json['total_results'] === "0") {
        html = "<p>No information available</p>";
    } else {
        console.log(json['results'][0]);
        html = '<ul><li><a href=""><img src="dist/img/info.png" style="opacity: 1;"></a><div style="opacity: 1; display: block;">';
        html += '<p>Title: ' + json['results'][0]['title'] + '</p>';
        ...
        html += '</div></li>';
        html += '<li><a href=""><img src="dist/img/plot.png"></a><div>';
        html += '<p>Overview: ' + json['results'][0]['overview'] + '</p>';

        var poster = "";
        if ("" + json['results'][0]['poster_path'] !== "null") {
            poster += '</div></li>';
            poster += '<li><a href=""><img src="dist/img/poster.png"></a><div>';
            poster += '<p class="poster"><img src="' + 'http://image.tmdb.org/t/p/w500/' + json["results"][0]["poster_path"] + '"></p>';
            html += poster;
            html += '</div></li></ul>';
        } else{
            html = "<p>No information available</p>";
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default()("a").click(function (event) {
        jquery__WEBPACK_IMPORTED_MODULE_0___default()("a").children("img").css("opacity", 0.3);
        jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).children("img").css("opacity", 1.0);
        ...
        jquery__WEBPACK_IMPORTED_MODULE_0___default.a.ajax({
            ...
            success: function(data) {
                ...
                if(json.ok) {
                    ...
                    var html = that.recreateTable(json);
...
Stars.prototype.message = function(message) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(this.sel+" .message").html(message).show().delay(2000).fadeOut(1000);
}

Stars.prototype.recreateTable = function(json) {
    var html = '<table><tr><th>&nbsp;</th><th>Title</th><th>Year</th><th>Rating</th></tr>';

    for (var i = 0; i<json['table'].length; i++) {
        html += '<tr><td><input type="radio" value="'+json['table'][i]['id']+'" name="id"></td>';
        html += '<td>'+json['table'][i]['title']+'</td><td>'+json['table'][i]['year']+'</td><td>';
        html += '<span class="stars">';
        if (json['table'][i]['rating'] === '0') {
            html += "not rated";
        }
        else {
            for (var x = 0; x < json['table'][i]['rating']; x++) {
                html += '<img src="dist/img/star-green.png">';
            }
            for (var y = 0; y < 10 - json['table'][i]['rating']; y++) {
                html += '<img src="dist/img/star-gray.png">';
            }
        }
        html += '</span> <span class="num">' + json['table'][i]['rating'] +'/10</span></td></tr>';
    }
    return html;
}
